...
def decode_aadhaar_qr(image_path: str) -> dict:
    ...
    return {
        ...
        "flag": "Cannot decode Aadhaar QR — possible lamination damage, poor scan quality, or unsupported dense QR format."
    }
...
def validate_aadhaar_number(aadhaar_number: str) -> dict:
    if not aadhaar_number:
        return {
            ...
            "severity": "HIGH",
            "description": "Could not extract Aadhaar number from image."
        }
...
def verify_name_match(qr_name: str, printed_name: str) -> dict:
    if not qr_name:
        return {"match": False, "confidence": 0, "flag": "NAME_MISSING_QR", "description": "No name found in QR code."}
    if not printed_name:
        return {"match": False, "confidence": 0, "flag": "EXTRACTION_FAILED", "description": "Could not extract name from document."}
    ...
    match_count = sum(1 for token in qr_tokens if token in printed_tokens)
    confidence = (match_count / max(len(qr_tokens), 1)) * 100
    
    if confidence >= 80:
        return {"match": True, "confidence": round(confidence, 2), "note": "Name found in document"}
    elif confidence >= 50:
        return {
...
def match_selfie_with_document(selfie_path: str, aadhaar_image_path: str) -> dict:
    if not FACE_REC_AVAILABLE:
        print("[MOCK] DeepFace not installed.")
        return {
            "match": False,
            "flag": "DEPENDENCY_MISSING",
            "reason": "DeepFace not installed.",
            "verdict": "Selfie match skipped due to missing facial recognition dependency."
        }
...
def run_aadhaar_verification(aadhaar_image_path: str, selfie_path: str, ocr_extracted_fields: dict, itr_address: str = None) -> dict:
    ...
    # --- Text Extraction (provided by Gemini OCR in orchestrator) ---
    printed_number = ocr_extracted_fields.get("aadhaar_number", "")
    printed_name = ocr_extracted_fields.get("name", "")
    
    if printed_number:
        printed_number = printed_number.replace(" ", "").replace("-", "").strip()
    
    if not printed_number:
        flags.append({"flag": "EXTRACTION_FAILED", "description": "Could not extract Aadhaar number from document.", "severity": "HIGH"})
        risk_score += 30

    # Layer 0: Image Forensics (ELA)
    if aadhaar_image_path and os.path.exists(aadhaar_image_path):
        ...
        if not qr_result["success"]:
            ...
            risk_score += 15  # QR is hard to read on most phone photos, don't auto-fail
    else:
        qr_result = {"success": False, "flag": "IMAGE_NOT_FOUND"}
        flags.append({"flag": "QR_UNREADABLE", "severity": "CRITICAL", "description": "Cannot decode QR code from PDF or missing image."})
        risk_score += 15
    ...
    if not number_check["valid"]:
        ...
        if number_check.get("flag") != "AADHAAR_NOT_FOUND":
            risk_score += 25  # Only add extra penalty if it WAS found but invalid (Verhoeff fail)
    ...
    return {
        "verified": risk_score < 40,
        "aadhaar_number": printed_number,
