...
def process_verification(input_data, mock_aa_data=None, is_image=True):
    ...
    if has_critical_veto:
        bucket = 3
        status = "Forged"
        description = "Critical anomalies detected (AUTO REJECT)."
    elif final_score <= 30:
        bucket = 1
        status = "Verified"
        description = "Salary slip passed structural and statutory DNA checks."
    elif final_score <= 65:
        bucket = 2
        status = "Requires Review"
        description = "Suspicious formatting or statutory violations found."
    else:
        bucket = 3
        status = "Forged"
        description = "High fraud score detected across multiple DNA layers."

    return {
        "bucket": bucket,
        "status": status,
        "description": description,
        "differences": all_issues,
        "data": {
            "routing": {
...
def run_salary_slip_dna_analysis(file_path: str, is_pdf: bool, extracted_metadata: Dict[str, Any]) -> Dict[str, Any]:
    """
    Standardized orchestrator for Salary Slip DNA analysis.
    """
    logger.info("Starting Salary Slip DNA Analysis...")
    try:
        # extracted_metadata might contain things extracted by Gemini, but we also run the local layers.
        # Salary slip expects input_data as file_path if is_image=True
        # For this hackathon, we assume the file_path points to an image (PDFs are rasterized before this)
        raw_result = process_verification(input_data=file_path, mock_aa_data=None, is_image=True)
        return raw_result
    except Exception as e:
        logger.error(f"Salary Slip DNA Error: {e}")
        return {
            "bucket": 3,
            "status": "Error",
            "description": "Failed to complete Salary Slip DNA analysis.",
            "differences": [str(e)]
        }
